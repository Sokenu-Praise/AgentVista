"""
ImageSearch tool - Image search using Serper.dev Google Images Search API.
Supports text-to-image search and reverse image search.
"""
...
'''
    parameters = {
        ...
        "required": []  # at least one of query or image_url is required
    }
    ...
    def __init__(self, config=None):
        ...
        self.api_endpoint = "https://google.serper.dev/images"
        self.reverse_image_endpoint = "https://google.serper.dev/lens"
        self.timeout = config.get('timeout', 100) if config else 100
        self.download_image_counter = 0

        # Search image compression config
        args_obj = config.get('args') if config and isinstance(config, dict) else None
        if args_obj:
            # args is a Namespace object; use getattr to access attributes
        ...
        # Search images use stricter compression (default: half of global limit)
        ...
        self.search_image_min_pixels = global_min_pixels
    ...
    def _upload_local_image(self, image_path, max_retries=3, timeout=30):
        """
        Upload a local image to an image hosting service.
        Tries multiple services in priority order with automatic fallback.
        ...
        def parse_0x0_response(response):
            """Parse 0x0.st response (returns plain-text URL)."""
            try:
        ...
        def parse_catbox_response(response):
            """Parse catbox.moe response (returns plain-text URL)."""
            try:
                url = response.text.strip()
                if url.startswith('http://') or url.startswith('https://'):
                    return url
        ...
        def parse_cloudflareimg_response(response):
            """Parse cloudflareimg.cdn.sn JSON response."""
            try:
                result = response.json()
                if result.get('success'):
        ...
        # Image hosting services in priority order
        upload_services = []
        
        # 1. ImgBB (requires API key, most reliable)
        ...
        if imgbb_key:
            upload_services.append({
                ...
                'extra_data': {'key': imgbb_key}
            })
        
        # 2. cloudflareimg.cdn.sn (webp compression, no API key needed)
        upload_services.append({
            'name': 'cloudflareimg.cdn.sn',
            'url': 'https://cloudflareimg.cdn.sn/api/v1.php',
            'files_key': 'image',
            'parse_response': parse_cloudflareimg_response,
            'extra_data': {'outputFormat': 'webp'}
        })
        
        # 3. Anonymous services (no API key needed)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Compress using stricter parameters for search images
                    ...
                    # Save with lower quality to further reduce file size
    ...
    def call(self, params, **kwargs):
        """
        Execute image search (text-to-image or reverse image search).

        Args:
            params: Dict with search_type, query, or image_url.
            **kwargs: May include save_dir for downloading images.
        ...
        if isinstance(params, str):
            try:
                ...
                # If parsing fails, treat as a query string
    ...
    def _text_to_image_search(self, params, max_results, save_dir=None):
        ...
        Args:
            params: Dict containing query.
            max_results: Maximum number of results.
            save_dir: If provided, download images to this directory.
        ...
        try:
            ...
            # Send request with proxy and retry support
            max_retries = 3
            retry_delay = 1
            response = None
            ...
            # Format output (WebWatcher style: plain-text URLs, no markdown image refs)
            formatted = []
    ...
    def _reverse_image_search(self, params, max_results, save_dir=None):
        ...
        Args:
            params: Dict containing image_url.
            max_results: Maximum number of results.
            save_dir: If provided, download images to this directory.
        ...
        try:
            ...
            # Check if it's a local file path (exclude URLs and data URIs)
            if not image_url.startswith('http://') and not image_url.startswith('https://') and not image_url.startswith('data:'):
                # Could be a local file path; check if it exists
            ...
            # Send request with proxy and retry support
            # Note: use data=json.dumps(payload) instead of json=payload for consistency
            max_retries = 3
            retry_delay = 1
            response = None
            ...
            # Serper.dev /lens response: organic results array with
            # title, source, link, imageUrl, thumbnailUrl per entry
            
            # Extract all types of matching results
            all_results = []
            
            # Prefer Serper.dev organic array format
            ...
            # Fallback to other formats (SerpAPI compatibility)
            if not all_results:
                # Visual matches (SerpAPI format)
