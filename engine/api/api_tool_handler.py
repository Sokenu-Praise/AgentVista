...
# Global prompt variables (will be set by api_processors.py or api_model_caller.py)
FEEDBACK_PROMPT_TOO_SMALL = ""
TOOL_CALL_CROP_MULTI_TRUN_PROMPT = ""


def set_global_prompts(feedback_too_small, tool_call_prompt):
    """Set global prompt variables for API tool handler."""
...
class APIToolHandler:
    ...
    def _determine_image_name(self, tool_name, image_ref, alt_text, node):
        """Resolve image name from tool output; support tool_image_N and observation_N."""
    ...
    def process_tool_output_for_images(self, tool_result, tool_name, node):
        ...
        """
        # Skip image extraction for image_search and visit tools (aligned with WebWatcher design)
        # image_search returns plain text URLs instead of markdown image references
        # visit returns text summaries, even if LLM sometimes includes image references in summary
            ...
            try:
                ...
                if original_filename:
                    ...
                    if not os.path.exists(original_path) and os.path.exists(standard_path):
                        # Copy a copy for model to access via original name
                        try:
                            import shutil
    ...
    def prepare_tool_kwargs(self, tool_name, node):
        ...
        if tool_name == 'code_interpreter':
            # Pass image_map directly from node (API mode doesn't use observations)
            tool_kwargs['image_map'] = node.image_map.copy()
        
        if tool_name == 'zoom':
            # Pass image_map directly from node for zoom tool to use
            tool_kwargs['image_map'] = node.image_map.copy()
    ...
    def execute_tool_call(self, tool_name, parameters, node, turn_idx, tool_call_id=None):
        ...
        if not TOOLS_AVAILABLE:
            ...
            return {
                ...
                'feedback_messages': [{
                    "role": "user",
                    "content": [{"type": "text", "text": feedback_msg}]
                }],
                'error': True
            }
            ...
            if new_images:
                for image_name, pil_image in new_images:
                    ...
                    if not os.path.exists(image_path):
                        ...
                        # print(f"[Image Skip] {image_name} already exists, skipping save")
                        pass
                    
                    # Save image info to trajectory
                    if self.save_dir:
                        try:
                            image_info_event = {
                                "turn_idx": turn_idx,
