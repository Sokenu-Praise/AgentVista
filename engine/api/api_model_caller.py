"""
Model caller for API-based TTS search strategies.
"""
...
from engine.api.api_caller import call_vision_api
from engine.api.api_tool_handler import APIToolHandler
import json

# Global prompt variables (will be set by infer_api.py)
# Note: These are now set in api_tool_handler, but we keep them here for backward compatibility
FEEDBACK_PROMPT_TOO_SMALL = ""
TOOL_CALL_CROP_MULTI_TRUN_PROMPT = ""


def set_global_prompts(feedback_too_small, tool_call_prompt):
    """
    Set global prompt variables for API model caller.
    Also sets them in api_tool_handler.
    
    Args:
        feedback_too_small: Feedback for too small bbox
        tool_call_prompt: Tool call prompt template
    """
    global FEEDBACK_PROMPT_TOO_SMALL, TOOL_CALL_CROP_MULTI_TRUN_PROMPT
...
def create_model_caller(args, sampling_params, save_dir=None):
    """
    Create a model caller function for the TTS search strategies.
    ...
    """
    import sys
    # print(f"[DEBUG] create_model_caller called! save_dir={save_dir}", file=sys.stderr, flush=True)
    ...
    # print(f"[Function Calling] ENABLE_FUNCTION_CALLING = {enable_fc}", file=sys.stderr, flush=True)
    ...
    if function_calling_enabled:
        try:
            ...
            if enabled_tools_env:
                ...
                # print(f"[Function Calling] ENABLED_TOOLS={enabled_tools_env}, filtered tools: {tool_names}")
            else:
            ...
            # print(f"[Function Calling] Building schema for tools: {tool_names}", file=sys.stderr, flush=True)
            tools_schema = build_openai_tools_schema(tool_names)
            # print(f"[Function Calling] Initialized with tools: {tool_names}", file=sys.stderr, flush=True)
            # print(f"[Function Calling] Using OpenAI-compatible format", file=sys.stderr, flush=True)
            # print(f"[Function Calling] Schema length: {len(tools_schema) if tools_schema else 0}", file=sys.stderr, flush=True)
            # if tools_schema:
                # import json
                # print(f"[Function Calling] Schema preview: {json.dumps(tools_schema[0] if len(tools_schema) > 0 else {}, indent=2, ensure_ascii=False)[:500]}", file=sys.stderr, flush=True)
    ...
    def model_caller(node: SearchNode) -> str:
        """
        Call the vision API and process the response, handling zoom operations.
        ...
        try:
            ...
            # Extract text content and reasoning_details for logging and conversation history
            # For function calls, content may be None or empty, but we should still try to extract it
            if isinstance(response, dict):
                # Get content - may be None for function calls, but could contain reasoning text
                response_text = response.get("content") or ""
                # If content is None, convert to empty string
                if response_text is None:
                    response_text = ""
                
                # DEBUG: Log response structure for function calls
                # if "tool_calls" in response:
                #     print(f"[DEBUG] Function call response structure:")
                #     print(f"  - response type: {type(response)}")
                #     print(f"  - response keys: {list(response.keys())}")
                #     print(f"  - content: {repr(response_text[:200]) if response_text else 'None/Empty'}")
                #     print(f"  - content type: {type(response.get('content'))}")
                #     if "reasoning_content" in response:
                #         rc = response.get("reasoning_content")
                #         print(f"  - reasoning_content present: {type(rc)}, value: {repr(rc[:100]) if isinstance(rc, str) and rc else 'Empty/None'}")
                #     if "reasoning_details" in response:
                #         print(f"  - reasoning_details present: {type(response.get('reasoning_details'))}")
                #     if "reasoning" in response:
                #         print(f"  - reasoning present: {type(response.get('reasoning'))}")
            else:
                response_text = response if isinstance(response, str) else ""
            
            reasoning_details = None
            reasoning_text = None  # Human-readable reasoning for display
            ...
            if isinstance(response, dict):
                ...
                if "reasoning_content" in response:
                    ...
                    # if isinstance(reasoning_text, str) and reasoning_text.strip():
                        # Successfully extracted reasoning_content
                        # print(f"[Reasoning] Extracted reasoning_content: {len(reasoning_text)} chars")
                    # else:
                    if not isinstance(reasoning_text, str) or not reasoning_text.strip():
                        reasoning_text = None
                        # print(f"[Reasoning] No reasoning_content in tool call response (keys: {list(response.keys())}, content: {repr(response_text[:200]) if response_text else '(empty)'})")
                else:
                    # Debug: log when reasoning_content is missing
                    if "tool_calls" in response:
                        content_preview = response_text[:200] if response_text else "(empty)"
                        # print(f"[Reasoning] No reasoning_content in tool call response (keys: {list(response.keys())}, content: {repr(content_preview)})")
                ...
                # For function calls, if content exists but reasoning_text is empty, use content as reasoning
                # This handles cases where API returns reasoning in content field for function calls
            ...
            if isinstance(response, dict) and "tool_calls" in response:
                # Function call response - only store the FIRST tool_call (we only execute one)
                tool_calls = response["tool_calls"]
                first_tool_call_only = [tool_calls[0]] if tool_calls else []
                assistant_msg = {
                    "role": "assistant", 
            ...
            if save_dir:
                # Build display text similar to old version
                # For tool calls, combine reasoning text with tool call indicator
                if isinstance(response, dict) and "tool_calls" in response:
                    ...
                    # Priority: reasoning_text > response_text (content field)
                    display_parts = []
                ...
                # Add reasoning text to trajectory if available (for human readability)
                # if reasoning_text:
                #     assistant_turn_for_traj["reasoning_content"] = reasoning_text
            ...
            if action == "function_call" or action == "tool_call":
                ...
                # Get tool_call_id from the last assistant message if not provided
                ...
                # If skip_processing is True (zoom tool), return early
                if result.get('skip_processing'):
                    node.current_turn += 1
                    return response_text
            
            elif action == "answer":
                # Only set final_answer here, let the caller handle mark_final() after reflection
            ...
            import traceback
            error_msg = f"Error in model caller: {e}"
            print(error_msg)
            traceback.print_exc()
            # Save error to trajectory if save_dir is available
            # Note: save_dir is not directly available here, but we can log the error
            # The caller should handle saving the error state
            return None
